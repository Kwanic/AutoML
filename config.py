...
class Config:
    ...
    def __init__(self):
        ...
        # BO sample number: "ALL" for all data, or a number for specific sample count
        bo_sample_str = os.getenv("BO_SAMPLE_NUM", "ALL").strip()
        if bo_sample_str.upper() == "ALL":
            self.bo_sample_num = None  # None means use all data
        else:
            self.bo_sample_num = int(bo_sample_str)
